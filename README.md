...
| 대상 | 라이선스 |
|---|---|
| 사이트 코드 (Astro 설정·컴포넌트·레이아웃·스타일·스크립트) | MIT — [`LICENSE`](LICENSE) |
| 글·이미지 (`src/content/notes`) | 저작권 유보 — [`CONTENT-LICENSE.md`](CONTENT-LICENSE.md) |

출처와 원문 링크를 표시한 짧은 인용은 괜찮습니다. 전문 재게시·번역·2차 가공·상업적 이용과
AI 학습 데이터로의 사용은 사전 서면 허락이 필요합니다. 저장소가 공개되어 있다는 사실이
글에 대한 이용 허락을 뜻하지는 않습니다.
